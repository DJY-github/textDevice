...
Device::Device(void)
{
	...
	memset(one_online_status_, -1, sizeof(one_online_status_));
}
...
int Device::get(const CmdFeature& cmd_feature, Tags& tags)
{
	...
	if(0x03 == funcation || 0x04 == funcation)
	{
		...
		for(it = tags.begin(); it != tags.end(); ++it)
		{
            ...
            if(1 == one_online_status_[atoi(param3.c_str())] || 1 == one_online_status_[0])
            {
		...
		for(it = tags.begin(); it != tags.end(); ++it)
		{
			...
			if(TagValue::kDigit == (*it)->conf.data_type)
			{
				...
				if(is_ok)
				{
                    ...
                    if(atoi(param3.c_str())>=0 && atoi(param3.c_str())<39)
                    {
                        one_online_status_[atoi(param3.c_str())] = val;
                    }

                    if(1 == one_online_status_[0])
                    {
                        (*it)->value.quality	= util::RTData::kOutOfValRange;
                    }
                    else
