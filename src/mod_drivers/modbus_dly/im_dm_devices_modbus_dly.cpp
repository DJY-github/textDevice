#include "im_dm_devices_modbus_dly.h"
...
int Device::get(const CmdFeature& cmd_feature, Tags& tags)
{
	...
	if(!serial->isOpen())
	{
		if(!serial->reOpen())
		{
		    setUQuality(util::RTData::kCommDisconnected);
			return util::RTData::kCommDisconnected;
		}
	}

	if(cmd_feature.cmd.empty())
    {
        return virtualCmd(tags);
    }

 	util::IMBuffer<16> cmd;
	if(!cmd.fromHex(cmd_feature.cmd.c_str()))
	{
	    setUQuality(util::RTData::kConfigError);
	...
	if(0 >= read_len)
	{
	    setUQuality(util::RTData::kCmdNoResp);
		return util::RTData::kCmdNoResp;
	}
	if(packet_recv.preRecvLen() != read_len)
	{
		serial->close();
		setUQuality(util::RTData::kCmdRespError);
	...
	if(0x03 == funcation || 0x04 == funcation)
	{
		...
		for(it = tags.begin(); it != tags.end(); ++it)
		{
			...
			if(0 == (*it)->conf.name.find("CT"))
            {
                if(util::RTData::kOk == (*it)->value.quality)
                {
                    CT_ = (*it)->value.pv.toFloat();
                }
            }
		}
	}
					...
					const libistring& param1, const libistring& param2, const libistring& param3)
{
	...
	if(TagValue::kAnalog == data_type || TagValue::kEnumStatus == data_type)
	{
        ...
        if(std::string::npos != param3.find("CT"))
		{
		    if(CT_ < 0)
            {
                re_value.quality	= util::RTData::kNeedWait;
            }
			else
            {
                re_value.pv = util::IMVariant(re_value.pv.toDouble()*CT_);
            }
		}
		else if(0 == param3.find("Ua"))
        {
            U_[0] = re_value;
        }
        else if(0 == param3.find("Ub"))
        {
            U_[1] = re_value;
        }
        else if(0 == param3.find("Uc"))
        {
            U_[2] = re_value;
        }
	}
...
void Device::setUQuality(util::RTData::Quality quality)
{
    for(int i=0; i<3; i++)
    {
        U_[i].quality  = quality;
    }
}

int Device::virtualCmd(Tags& tags)
{
        Tags::iterator it;
		for(it = tags.begin(); it != tags.end(); ++it)
		{
		    ...
		    if("onoff" == param3)
            {
                if(util::RTData::kOk == U_[0].quality)
                {
                    if(0 == U_[0].pv.toUInt() )
                    {
                        on_off_ = false;
                    ...
                    (*it)->value.pv = util::IMVariant(on_off_);
                    (*it)->value.quality	= util::RTData::kOk;
                }
                else
                {
                    (*it)->value.quality	= U_[0].quality;
                }
            }
		}
