...
bool ModbusPacketRecv::isCRCOk() const
{
	if(kRtu == packet_send_.type_)
	{
		uint16_t crc = ModbusUtil::crc16((uint8_t*)recv_buf_, recv_data_byte_ + 3);
			if((uint8_t)recv_buf_[recv_data_byte_+3] == (crc >> 8) &&
				(uint8_t)recv_buf_[recv_data_byte_+4] == (crc & 0x00FF))
			{
				return true;
			}
