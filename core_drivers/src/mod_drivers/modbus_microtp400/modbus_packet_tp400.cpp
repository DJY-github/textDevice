...
int ModbusPacketRecv::recvLen()
{
	...
	{
		int recv_byte_ = recv_buf_[5];
		recv_data_byte_ = recv_byte_ - 3;
		return recv_byte_;
	}
...
bool ModbusPacketRecv::isFunCodeOk() const
{
    ...
    {
        fun = recv_buf_[recv_data_offset_-2];
    }
...
{
	int data_byte = 2;
	int	pos = recv_data_offset_ + (data_attr - packet_send_.data_addr_)*2;  //recv_data_offset_后面才是真正的数据位
