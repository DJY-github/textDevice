...
float    ModbusUtil::toFloat(uint8_t* byte,  bool is_inverse)
{
	float re = 0.0f;
	char tmp[4] = {0};
...
void ModbusPacketSend::buildPacket(PacketType type, int device_addr, int function , int data_addr, int data_count)
{
	...
	if(kRtu == type_)
	{
		send_buf_[0] = (device_addr >> 8) & 0xFF ;
		send_buf_[1] = function;
		send_buf_[2] = device_addr & 0xFF;
		send_buf_[3] = data_addr >> 8;
		send_buf_[4] = data_addr & 0x00FF;
		send_buf_[5] = data_count >> 8;
		send_buf_[6] = data_count & 0x00FF;

		uint16_t crc = ModbusUtil::crc16((uint8_t*)send_buf_, 7);
		send_buf_[7] = crc >> 8;
		send_buf_[8] = crc & 0x00FF;
	}
	else
	{
...
bool ModbusPacketRecv::isCRCOk() const
{
	if(kRtu == packet_send_.type_)
	{
		uint16_t crc = ModbusUtil::crc16((uint8_t*)recv_buf_, recv_data_byte_ + 4);
...
{
    unsigned char fun = recv_buf_[recv_data_offset_-3];
    if( 0x80 == (fun & 0x80) )
    {
        return false;
    }
...
{
    if(packet_send_.send_buf_[0] != recv_buf_[0] || packet_send_.send_buf_[1] != recv_buf_[1] || packet_send_.send_buf_[2] != recv_buf_[2])
    {
        return false;
    }
...
{
	int data_byte = 2;
	int	pos = recv_data_offset_ + (data_attr - packet_send_.data_addr_)*2;  //recv_data_offset_���������������λ
