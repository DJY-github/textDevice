...
int Device::get_cpu_used(){
    Oid sto_table("1.3.6.1.2.1.25.3.3");
    vec_table res;
    if(! Get_Table(sto_table, res) || res.size() <= 1){
        return -1;
    }
    int sum=0;
    unsigned int i=0;
    for ( ;i<res[1].size();i++){
//      cout<<"cpu No."<<res[0][i].second<<" used: "<<res[1][i].second<<"\%"<<endl;
        sum+=atoi(res[1][i].second.c_str());
    }
//  cout<<"CPU USED :  "<<sum/i<<"\%"<<endl;
    if (i > 0) return sum/i;
    else return -1;
}

int Device::get_mem_used(){
    Oid sto_table("1.3.6.1.2.1.25.2.3");
    vec_table res;
    if (! Get_Table(sto_table, res) || res.size() <= 4){
        return -1;
    }
    float Ram_size = 0,Ram_used = 0;
    for (unsigned int i = 0; i < res[1].size(); i++) {
        if (res[1][i].second == "1.3.6.1.2.1.25.2.1.2") {
            Ram_size += atof(res[4][i].second.c_str());
            Ram_used += atof(res[5][i].second.c_str());
        }
        if (res[2][i].second == "Cached memory" || res[1][i].second == "Memory buffers"){
            Ram_used -= atof(res[5][i].second.c_str());
        }
    }
//    cout<<"  memory  :  size ="<<Ram_size<<"     used_size(-cache-buffers) ="<<Ram_used<<"  used_Percent ="<<(Ram_used/Ram_size)*100<<endl;
    if (Ram_size > 0) return (int)(Ram_used / Ram_size*100);
    else return -1;
}

int Device::get_Disk_used(){
    Oid sto_table("1.3.6.1.2.1.25.2.3");
    vec_table res;
    if ( !Get_Table(sto_table,res) || res.size() <= 4) {
        return -1;
    }
    float disk_size = 0,disk_used = 0;
    for (unsigned int i = 0; i < res[1].size(); i++){
        if (res[1][i].second == "1.3.6.1.2.1.25.2.1.4" ||
           res[1][i].second == "1.3.6.1.2.1.25.2.1.7")
        {
            disk_size += atof(res[4][i].second.c_str());
            disk_used += atof(res[5][i].second.c_str());
        }
    }

    //cout<<"  Disk  :  size ="<<disk_size<<"     used_size ="<<disk_used<<"  used_Percent ="<<(disk_used/disk_size)<<endl;
    if (disk_size > 0) return (int)(disk_used / disk_size*100);
    else return -1;
}
...
int Device::get(const CmdFeature& cmd_feature, Tags& tags)
{
    ...
    for (it = tags.begin(); it != tags.end(); ++it)
    {
        ...
        if (strcmp((*it)->conf.get_param1.c_str(), "cpu") == 0)
        {
            if((res = get_cpu_used()) == -1){
         ...
         else if (strcmp((*it)->conf.get_param1.c_str(), "mem") == 0)
        {
            if((res = get_mem_used()) == -1){
               // cout<<"------------data out of value range-----------"<<endl;
               (*it)->value.quality	= util::RTData::kCmdNoResp;
                close();
                return util::RTData::kCmdNoResp;;
         ...
         else if (strcmp((*it)->conf.get_param1.c_str(), "hd") == 0)
        {
            if((res = get_Disk_used()) == -1){
               // cout<<"------------data out of value range-----------"<<endl;
               (*it)->value.quality	= util::RTData::kCmdNoResp;
                close();
                return util::RTData::kCmdNoResp;;
